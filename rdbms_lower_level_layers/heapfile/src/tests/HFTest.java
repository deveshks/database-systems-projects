...
class HFDriver extends TestDriver implements GlobalConst
{
	...
	//	choice = 1400;      // big enough for file to occupy > 1 data page
	...
	//deal with variable size records. 
	protected boolean test5 () {
		...
		if ( status == OK ) {
			...
			for (int i =0; (i < choice) ; i++) {
				...
				DummyRecord rec = new DummyRecord(reclen *(i%3+1));
				rec.ival = i;
				rec.fval = (float) (i*2.5);
				rec.name = "record" + i;
		...
		if ( true ) {
			...
			while (!done) { 
				...
				if (!done) {
					...
					String name = ("record" + rec.ival  );

					
					
					if (Math.abs((rec.fval - (float)(rec.ival)*2.5))>.01
							|| !name.equals(rec.name))
...
//
//						System.out.println ("*** Record " + rec.ival  
//								+ " matchs  what we inserted\n"); 
					}
					
					
					len = tuple.getLength();
					if ( len!=reclen*(rec.ival%3+1)) {
						System.err.println ("*** Record " + i + " had unexpected length " 
								+ len + "\n");
						status = FAIL;
						break;
...
//						System.out.println ("*** Record size for " + rec.ival  
//								+ " matchs  what we inserted\n"); 
					}
				}	
				++i;
	...
	protected boolean test4 () {
		...
		if ( status == OK ) {
		 ...
		 try{ // added by ar
	            ...
	            System.out.println("HeapScan.close() failed.");
	        }
